...
present = ["harness_std","sortarray","binsearch_std"]
...
subprocess.call(["../cbmc/cbmc " + pstring  + mutated + ".c" + 
                 " --bounds-check --slice-formula --unwind " + unwind],
...
for f in glob.glob("mutant*" + mutated + ".c"):
    ...
    if True:
        resultF = open(results,'w')
        subprocess.call(["../cbmc/cbmc " + pstring  + f + 
                         " --bounds-check --slice-formula --unwind " + unwind],
                        shell=True, stdout = resultF, stderr = resultF)
        resultF.close()
    normal = False
