...
void fake_acquire_cpu_only(void)
{
#ifdef CBMC
...
{
	fake_acquire_cpu_only();
#ifndef FORCE_FAILURE_DYNTICKS
	rcu_idle_exit();
#endif
}

void fake_acquire_irq(void)
{
	fake_acquire_cpu_only();
#ifndef FORCE_FAILURE_IRQ
	rcu_irq_enter();
#endif
}

void fake_release_cpu_only(void)
{
#ifdef CBMC
...
{
	rcu_idle_enter();
	fake_release_cpu_only();
}

void fake_release_irq(void)
{
	rcu_irq_exit();
	fake_release_cpu_only();
}
...
#ifndef FORCE_FAILURE_IRQ_BOGUS
	rcu_irq_enter();
#endif
#ifdef DFORCE_FAILURE_IRQ_BOGUS_2
	rcu_irq_enter();
#endif
	rcu_irq_exit();
	fake_release_cpu();
...
int main(int argc, char *argv[])
{
	...
	__CPROVER_assume(__unbuffered_tpr_y == 0 || __unbuffered_tpr_x == 1 ||
	       CK_NOASSERT());

	assert(total_covered < COVTARGET);
	return 0;
}
