...
\ref{fig:harness} shows a CBMC harness for inserting elements in
sorted order into a doubly linked list.  This is a simple program, but typical of a
small verification problem:  it is not easy to write an obviously
bug-free implementation of this routine, and it is non-standard enough
that no library implementation is likely available.
...
\caption{Harness for insertion sort into a doubly linked list.}
\label{fig:harness}
\end{figure}
...
the harness shown in Figure \ref{fig:harness}.  If we take the
...
limit to size 4, again the same 8 mutants survive (though this time
the verification requires 229 minutes).  We call an
