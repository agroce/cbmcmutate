...
plausible but weak harness, and checked whether 1)
surviving mutants revealed the problem, 2) the trace output
...
knowledge) harness that mutant suvivals could detect.  The existence of
...
version of the harness, but checked out existing harness using
mutants, and determined that all 3 surviving mutants are equivalent to
a true binary search.

Checking harness mutants produced results confirming the belief this is a
...
removal of an entire conditional branch.  A harness check shows that of
the 105 compiling harness mutants, 92 fail to verify the original
code.  Another 2 have a worse kill rate than the original (which kills
81\% of mutants, a low rate due to the code redundancy), and 11
survive.  The large number of survivors is due to a redundancy of the
final harness, which checks sortedness and the permutation property
for both a forward {\tt next} traversal of the list and a {\tt prev}
traversal.  Omitting any \emph{one} of these can still detect all
mutants.  Removing two, however, fails to kill mutants.  The two
harness mutants with worse kill rates have such poor kill rates ($<50$\% and $<25$\% respectively) that they do not show possible
erroneous versions of the harness.
