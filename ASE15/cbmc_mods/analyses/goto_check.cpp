...
public:
  ...
  {
    enable_find_success=_options.get_bool_option("find-success");
  ...
  const guardt &guard)
{
  ...
  std::string expr_string=from_expr(ns, "", expr);
  ...
  if(assertions.insert(new_expr).second)
  {
    ...
    if (enable_find_success) {
      printf ("SWITCHING TO ASSUME: %s\n", expr_string.c_str());
      type = ASSUME;
    }
